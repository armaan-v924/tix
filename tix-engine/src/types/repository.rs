...
impl Repository {
    ...
    pub fn git_clone(&self) -> Result<PathBuf, TixError> {
        ...
        let git_repo = git2::Repository::clone(&self.remote, &self.code_path).map_err(|e| {
        ...
        info!(alias = %self.alias, path = %path.display(), "clone complete");
        Ok((path))
    }

    fn git_repo(&self) -> Result<git2::Repository, TixError> {
        if !&self.is_cloned {
            error!(alias = %self.alias, "attempted to open repo that has not been cloned");
            return Err(TixError::from("The repository has not yet been cloned."));
        }
        git2::Repository::open(&self.code_path).map_err(TixError::Git)
    }
    ...
    pub fn sync_base(&self, branch: &str, force: bool) -> Result<(), TixError> {
        info!(alias = %self.alias, branch, force, "syncing repository");
        let repo = self.git_repo()?;
        ...
        if !force {
            let statuses = repo.statuses(None).map_err(TixError::Git)?;
        ...
        let mut remote = repo.find_remote("origin").map_err(TixError::Git)?;
        remote.fetch(&[branch], None, None).map_err(TixError::Git)?;
        ...
        if repo.find_branch(branch, git2::BranchType::Local).is_err() {
            ...
            let commit = repo
                ...
                .map_err(TixError::Git)?;
            let target = repo.find_commit(commit.id()).map_err(TixError::Git)?;
            // false = don't force-overwrite if it somehow already exists
            repo.branch(branch, &target, false).map_err(TixError::Git)?;
        ...
        let fetch_head = repo.find_reference("FETCH_HEAD").map_err(TixError::Git)?;
        ...
        let worktrees = repo.worktrees().map_err(TixError::Git)?;
        for name in worktrees.iter().filter_map(|n| n.ok().flatten()) {
            // open the worktree as its own Repository so we can inspect its HEAD
            let wt = repo.find_worktree(name).map_err(TixError::Git)?;
            let wt_repo = git2::Repository::open(wt.path()).map_err(TixError::Git)?;
        ...
        repo.set_head(&ref_name).map_err(TixError::Git)?;
        repo.checkout_head(Some(git2::build::CheckoutBuilder::new().force()))
            .map_err(TixError::Git)?;

        // move the local branch pointer to the fetched remote commit
        let mut branch_ref = repo.find_reference(&ref_name).map_err(TixError::Git)?;
        if force {
        ...
        info!(alias = %self.alias, branch, "sync complete");
        Ok(())
    }
}
